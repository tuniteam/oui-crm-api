...
export class ActivationService {
  ...
  async complete(dto: ActivationCompleteDto): Promise<AuthTokensResponseDto> {
    ...
    const password = await assertAndHashPassword(this.config, dto.password);
    const now = new Date();
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      await this.audit.log(tx, {
        projectId: null,
        userId: user.id,
        action: AUTH_AUDIT.ACTIVATION_COMPLETE,
        objectType: AUDIT_OBJECTS.USER,
        objectId: user.id,
        metadata: { consents: [LegalDocument.CGU, LegalDocument.RGPD] },
      });
    });
